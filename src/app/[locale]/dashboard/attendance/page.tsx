...
import { getAttendanceData, getMyEmployeeId } from '@/lib/actions/attendance-actions';
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const data = await getAttendanceData(month, year, view === 'self' ? (myEmployeeId || 'none') : undefined);
      setRecords(data);
      setLoading(false);
    }
    ...
    <div className="flex flex-col h-full bg-gray-50/50 dark:bg-black p-4 sm:p-6 lg:p-10 space-y-8 rounded-3xl">
      ...
      {loading ? (
        ...
        <>
            {view === 'self' ? (
                records.length > 0 ? (
                    <AttendanceCalendar record={records[0]} month={month} year={year} />
                ) : (
