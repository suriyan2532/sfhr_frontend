...
export default async function UnitListPage({
  ...
  const { units, totalCount } = await getUnits(query, currentPage);
    ...
    <div className="p-6 sm:p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="text-2xl font-bold text-gray-900">{t('categories.unit')}</h1>
                <p className="text-sm text-gray-500">Manage organizational units and agencies.</p>
            </div>
        </div>
        <Link
          href="/dashboard/settings/master/unit/create"
      ...
      {units.length > 0 ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-200 border-dashed shadow-sm">
            ...
            <h3 className="text-sm font-semibold text-gray-900">No units found</h3>
