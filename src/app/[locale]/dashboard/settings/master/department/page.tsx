...
export default async function DepartmentListPage({
  ...
  const { departments, totalCount } = await getDepartments(query, currentPage);
    ...
    <div className="p-6 sm:p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="text-2xl font-bold text-gray-900">{t('categories.department')}</h1>
                <p className="text-sm text-gray-500">Manage organizational departments and functional groups.</p>
            </div>
        </div>
        <Link
          href="/dashboard/settings/master/department/create"
