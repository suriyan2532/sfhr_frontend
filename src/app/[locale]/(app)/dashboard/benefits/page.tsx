...
import {
  ...
  getBenefitClaims,
  getBenefitTypes,
} from "@/lib/actions/benefit-actions";
  ...
  if (!user?.employeeId) {
    return (
      <div className="p-10 text-center">
        <div className="max-w-md mx-auto bg-amber-50/50 dark:bg-amber-900/10 backdrop-blur-xl p-8 rounded-4xl border border-amber-200/20 shadow-xl">
          ...
          <p className="text-amber-700 dark:text-amber-400 text-sm">
            Your user account is not linked to an employee profile. Please
            contact HR to set up your benefits.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 min-h-screen bg-transparent">
      <div className="space-y-10 p-8 lg:p-12 bg-white/40 dark:bg-black/20 backdrop-blur-3xl border border-white/20 dark:border-white/5 rounded-[2.5rem] shadow-2xl shadow-black/5 animate-in fade-in duration-500">
        ...
        <section className="space-y-6">
          <BenefitClaimList claims={claims} />
        </section>
      </div>
    </div>
  );
