...
import { positionSchema, PositionFormValues } from '@/lib/validators/organization-schema';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-8 rounded-xl border border-gray-200 shadow-sm max-w-2xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Position Code (Optional)</label>
          <input
            {...register('code')}
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Job Level</label>
          <input
            type="number"
            {...register('level')}
            className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
