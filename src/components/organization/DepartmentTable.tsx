...
import { deleteDepartment } from '@/lib/actions/organization-actions';
            ...
            <tr key={dept.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  <div className="flex flex-col">
                      <span className="font-medium">{dept.company.name}</span>
                      {dept.unit && <span className="text-xs text-indigo-500">{dept.unit.name}</span>}
                  </div>
              </td>
