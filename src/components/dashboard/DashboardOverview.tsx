...
export async function DashboardOverview({ data }: OverviewProps) {
    ...
    const activities = [
        ...data.activities.recentEmployees.map((emp: any) => ({
            ...
            description: `${emp.firstName} ${emp.lastName} has been added to the system.`,
            time: emp.createdAt
        })),
        ...data.activities.recentLeaves.map((leave: any) => ({
            type: 'LEAVE' as const,
            title: `Leave Approved`,
            description: `${leave.employee.firstName} for ${leave.leaveType.name}`,
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden group">
                        <div className="relative z-10">
                            <h3 className="text-2xl font-bold mb-2">Need to approve requests?</h3>
                            <p className="text-indigo-100 mb-6 max-w-sm">There are currently {data.stats.pendingApprovals} pending leave requests waiting for your review.</p>
                            <button className="bg-white text-indigo-600 px-6 py-3 rounded-xl font-bold text-sm hover:bg-indigo-50 transition-colors flex items-center gap-2">
                                Go to Approvals <ArrowUpRight className="h-4 w-4" />
                            </button>
                        </div>
                        {/* Decorative blobs */}
