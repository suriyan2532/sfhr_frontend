...
export const GlassCard = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return (
    <div className={cn("bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-6 sm:p-10", className)}>
      {children}
    </div>
  );
};
...
export const GlassContainer = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return (
    <div className={cn("relative min-h-screen w-full bg-[#1e1e1e] font-[Rubik,sans-serif] text-white", className)}>
       ...
       <div 
            ...
            style={{ filter: 'brightness(0.5) blur(10px)' }} // Increased blur for content pages
        />
        {/* Gradient Overlay */}
        <div className="fixed inset-0 z-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent pointer-events-none" />
...
export const GlassLabel = ({ htmlFor, children, className }: { htmlFor?: string; children: React.ReactNode; className?: string }) => {
  return (
    <label htmlFor={htmlFor} className={cn("block text-xs font-medium text-blue-200 uppercase tracking-wider mb-1.5 ml-1", className)}>
      {children}
    </label>
  );
};
      ...
      <div className="relative group w-full">
        <select
          ...
          className={cn(
            ...
            " [&>option]:bg-gray-800 [&>option]:text-white", // Fix dropdown option visibility
            error && "border-red-500/50 focus:ring-red-500/50",
            className
          )}
          {...props}
