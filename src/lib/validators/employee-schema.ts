...
export const employeeSchema = z.object({
  ...
  // Personal Info
  idCard: z.string().length(13, 'ID Card must be 13 digits').regex(/^\d+$/, 'ID Card must be numbers only'),
  birthDate: z.string().optional().nullable().transform((val) => val ? new Date(val) : null), // Handle date string from form
