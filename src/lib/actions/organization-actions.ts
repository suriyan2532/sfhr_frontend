...
import { companySchema, unitSchema, departmentSchema, positionSchema } from '@/lib/validators/organization-schema';
import { revalidatePath } from 'next/cache';
import { redirect } from 'next/navigation';

export async function getCompanies(query: string = '', currentPage: number = 1) {
    const pageSize = 10;
    ...
    try {
        ...
        return { companies, totalCount, totalPages: Math.ceil(totalCount / pageSize) };
    } catch (error) {
...
// DEPARTMENT ACTIONS
export async function getDepartments(query: string = '', currentPage: number = 1) {
    const pageSize = 10;
    const skip = (currentPage - 1) * pageSize;
    ...
    try {
        ...
        return { departments, totalCount, totalPages: Math.ceil(totalCount / pageSize) };
    } catch (error) {
...
// POSITION ACTIONS
export async function getPositions(query: string = '', currentPage: number = 1) {
    const pageSize = 10;
    const skip = (currentPage - 1) * pageSize;
    ...
    try {
        ...
        return { positions, totalCount, totalPages: Math.ceil(totalCount / pageSize) };
    } catch (error) {
